...
`teacher` 适合需要跨多轮持续学习的场景，尤其适合系统学习一个 GitHub repo，或者长期学习像 `LLM inference` 这样的领域知识，并持续跟踪 learner 当前进度、薄弱点和待学习目标。
...
Codex  : $teacher 帮我长期学习大模型推理，监督我当前进度和待学习目标
...
当前默认内置最完整的是 `LLM inference interview prep`，但 `teacher` 本身不局限于面试准备，也适合系统学习 GitHub repo 或其他技术领域。当前内置课程图覆盖：
